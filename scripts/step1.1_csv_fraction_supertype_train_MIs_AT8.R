...
for (i in seq_along(csv_files)) {
  ...
  data <- rbind(data, 
                "percent.AT8.positive.area" = as.numeric(metadata$`percent AT8 positive area`))
  
  library("cafr")
  
  
  # Find the mi 
  MI_list <- getAllMIWz(data, data["percent.AT8.positive.area", ], negateMI = T, sorting = T)
...
# save(all_MILists, file = paste0("/Users/lingyi/Documents/sea-ad/models/attractors/MI_full/MIs_", selectfolder, "_Full_AT8.RData"))
